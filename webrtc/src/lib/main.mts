...
import { WebRtc, startWebRTC, stopWebRTC } from '../mod/webrtc.mjs';
import * as mainApp from './main-ext.mjs';

/**
 * auth Login Access Token
 */
export function authLoginAccessToken(): void {
...
*/
export function getAuthFromQueryString(webrtcOptions: any, accessOptions: any): void {

    // get the query.
    let urlParams: URLSearchParams = new URLSearchParams(location.search);
    let uniqueID: string = "";
    let conferenceAppID: string = "";

    // get uniqueID
    let hasUniqueID: boolean = urlParams.has('u');
    if (hasUniqueID === true) {

        // get the value.
        let uniqueIDValue: string = urlParams.get('u');
        if (uniqueIDValue !== "") {
            // uniqueID
    ...
    // get applicationID
    let hasApplicationID: boolean = urlParams.has('a');
    if (hasApplicationID === true) {

        // get the value.
        let applicationIDValue: string = urlParams.get('a');
    ...
    let hasWebSocket: boolean = urlParams.has('ws');
    if (hasWebSocket === true) {

        // get the value.
        let websocketValue: string = urlParams.get('ws');
        if (websocketValue !== "") {
            // websocket server
    ...
    // get token
    let hasToken: boolean = urlParams.has('t');
    if (hasToken === true) {

        // get the value.
        let tokenValue: string = urlParams.get('t');
        if (tokenValue !== "") {
            // token
 ...
 */
export function initialiseWebRTC(webrtcOptions: any, state?: any): void {
 ...
 */
export function deinitializeWebRTC(webRtc: WebRtc): void {
    try {
 ...
 * @param {WebRtc}    webRtc the implementation.
 */
function callbackStartWebRTC(webRtc: WebRtc, state: any): void {
