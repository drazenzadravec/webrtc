...
// ../mod/webrtc.mjs
function startWebRTC(options, callback, state) {
  let webRtcImp = null;
  try {
    if (options.signallingURL !== "") {
      webRtcImp = new WebRtc(options);
    } else {
      logger("warn", "WebRTC Interface", "Has not been initialised, signalling URL does not exist.");
    }
  } catch (e) {
  ...
  if (webRtcImp) {
    try {
      ...
      logger("Error", "Could not initialise the WebRTC interface", e);
    }
  } else {
    logger("warn", "WebRTC Interface", "Has not been initialised");
  }
}
function stopWebRTC(webRtcImp) {
  if (webRtcImp) {
    try {
      webRtcImp.webrtc.close();
    } catch (e) {
      logger("Error", "Could not stop the WebRTC interface", e);
    }
  } else {
    logger("warn", "WebRTC Interface", "Has not been initialised");
  }
